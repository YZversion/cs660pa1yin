...
bool TupleDesc::compatible(const Tuple &tuple) const {
  ...
  return std::equal(types.begin(), types.end(),
                    tuple.fields.begin(),
                    [](const type_t &type, const field_t &field) {
                      return (type == type_t::INT && std::holds_alternative<int>(field)) ||
                             (type == type_t::DOUBLE && std::holds_alternative<double>(field)) ||
                             (type == type_t::CHAR && std::holds_alternative<std::string>(field));
                    });
}
